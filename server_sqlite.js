...
const db = require("./database_sqlite.js");
...
app.post("/auth/register", (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        const params = [username.toLowerCase(), hashedPassword, 'user'];db.run(sql, params, function(err) {
            if (err) {
...
app.post("/movies", authenticateToken, (req, res) => {
    ...
    if (!title || !director || !year) {
        return res.status(400).json({ error: "Semua field (title, director, year) harus diisi" });
    }
...
app.post("/directors", authenticateToken, (req, res) => {
    ...
    if (!name || !birthYear) {
        return res.status(400).json({ error: "Semua field (name, birthYear) harus diisi" });
    }
